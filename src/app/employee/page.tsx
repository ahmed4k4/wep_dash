  ...
  useEffect(() => {
    const unsubscribe = listenToAuth(async (user) => {
      if (user) {
        ...
        // For now, use email as creator identifier
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      ...
      try {
        ...
        console.error("[EmployeePage] Print failed:", printErr);
  ...
  return (
    <main className="flex flex-1 flex-col">
      ...
      <section className="flex flex-1 flex-col gap-4 p-6 lg:flex-row">
        ...
        {/* Right column: Current orders placeholder */}
